...
def normalizeApiUserid(apiUserid):
    return str(apiUserid).strip().lower()
...
def getAwsRegionFromSocketName():
    ...
    if "new2" in socketName:
        return "eu-west-2"

    if "new3" in socketName:
        return "eu-west-3"

    print("ERROR: Unable to determine AWS region from socket hostname: " + socketName)
    print("Expected hostname to contain new2 or new3")
    sys.exit(1)
...
def setupDBConnection(runEnv, runUser, parentOrg, apiUserid):
    ...
    try:
        ...
        dbUser = "aducpnet"
        dbPort = "3306"
        sslCa = "/cls/appl/env/truststore/" + region + "-bundle_RSA2048.pem"

        if not os.path.exists(sslCa):
            print("ERROR: SSL CA file does not exist: " + sslCa)
            sys.exit(1)
